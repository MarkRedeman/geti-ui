---
name: geti-ui
description: Build and document components in the @geti/ui design system with repository conventions, theming requirements, and docs-first validation workflow.
---
...
2. **Consumer-like imports in docs**
   - Import from `@geti/ui` and subpaths (e.g. `@geti/ui/icons`), not internal source file paths.

3. **Theming requirements**
   - Ensure examples/pages render under `ThemeProvider`.
   - Ensure stylesheet import is present where needed: `@geti/ui/styles.css`.
...
- When using form controls in docs examples, use controls from `@geti/ui`.

## Assets and icons

- Icons: `@geti/ui/icons`
- Assets: `@geti/ui/assets/*`
- Keep exports tree-shakeable (named exports, no giant default objects).

## Quick checklist

- [ ] Uses `@geti/ui` public imports
