...
```tsx preview
import { Text, TreeView, TreeViewItem as Item } from '@geti-ai/ui';
...
> [!WARNING]
> Deprecated: use `@geti-ai/blocks` media components (`MediaRow`/`MediaGrid`) for new implementations.
...
> [!WARNING]
> Deprecated: use `@geti-ai/blocks` media components (`MediaGrid`/`MediaColumn`/`MediaRow`) for new implementations.
