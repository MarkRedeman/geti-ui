...
This page shows how to compose `ManagedTab` and `OverflowableTabs` together for a dataset workflow:

- each selected tab can open a contextual management menu,
- tabs collapse into a `N more` picker when space is limited,
- add-dataset action stays in the same row,
- tab panels remain application-owned.
...
- Use `ManagedTab` when active tabs expose actions.
- Use `OverflowableTabs` when tab count can exceed available width.
- Combine both when you need dataset management + responsive tab overflow behavior.
