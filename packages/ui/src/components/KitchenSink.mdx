...
The `components/` directory contains every UI component exported by `@geti-ai/ui`. Components are organised into 8 domain groups, each in its own subdirectory. This maps directly to the 8 export groups in `src/index.ts`.

| Subdirectory    | Group name         | Key Components                                         |
| :-------------- | :----------------- | :----------------------------------------------------- |
| `ui/`           | Primitive Actions  | Buttons, Avatar, Image, View, Divider                  |
| `form/`         | Form Controls      | Text inputs, Checkbox, Radio, Switch, Slider           |
| `form/pickers/` | Form Pickers       | Picker, ComboBox, Date/Calendar suite, Color suite     |
| `overlays/`     | Overlays           | Dialog, AlertDialog, Popover, Tooltip, ContextualHelp  |
| `navigation/`   | Navigation         | Tabs, Menu, ActionMenu, Breadcrumbs, Link              |
| `feedback/`     | Status & Feedback  | Loading, Progress, Meter, Skeleton, Toast, InlineAlert |
| `data/`         | Data Display       | TableView, ListView, CardView, TagGroup, ActionBar     |
