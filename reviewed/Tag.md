...
| 2   | 🟠 Medium | **No semantic role on the container `<div>`.** The tag renders as a generic `<div>` with no ARIA role. If it represents a status or category label, it should have `role="status"` or be wrapped in a list item. If it's interactive (has `onClick`), it needs `role="button"` and keyboard handlers. Currently it has neither.                                                                                                                 |
...
| 2   | 🔴 Medium | **`aria-label` test is testing wrong behavior.** `it('has aria-label set to the text')` tests `screen.getByLabelText('Accessible Tag')` — this works because `aria-label` is set on the div, but as noted above, this is incorrect a11y practice. The test will need to change once the `aria-label` is removed. |
