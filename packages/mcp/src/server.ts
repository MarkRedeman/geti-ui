...
export async function startServer(version: string): Promise<void> {
    ...
    'list_geti_ui_pages',
    {
      ...
      inputSchema: {
        includeDescription: z
          .boolean()
          .optional()
          .describe('Include a one-line description for each page'),
      },
    },
  ...
  server.registerTool(
    'get_geti_ui_page_info',
    {
      title: 'Get page info',
      description:
        'Returns page name, description, and list of ## section titles for a given page.',
      inputSchema: {
        page_name: z
          .string()
          .describe(
            'Page name or path (e.g. "Button", "components/ui/Button")',
          ),
      },
    },
    async ({ page_name }) => {
    ...
    'get_geti_ui_page',
    {
      ...
      inputSchema: {
        ...
        section_name: z
          .string()
          .optional()
          .describe('Optional: specific ## section to return'),
      },
    },
    async ({ page_name, section_name }) => {
      ...
      if (!section) {
        section = sections.find(
          (s) => s.name.toLowerCase() === section_name.toLowerCase(),
        );
      }
      if (!section) {
    ...
    'list_geti_ui_components',
    {
      ...
      inputSchema: {
        category: z
          ...
          .optional()
          .describe(
            'Filter by category (e.g. "ui", "form", "form/date-controls")',
          ),
      },
    },
    async ({ category }) => {
  ...
  server.registerTool(
    'get_geti_ui_component_props',
    {
      ...
      inputSchema: {
        component_name: z
          .string()
          .describe('Component name (e.g. "Button", "TextField")'),
      },
    },
    async ({ component_name }) => {
      ...
      return {
        content: [
          { type: 'text' as const, text: JSON.stringify(props, null, 2) },
        ],
      };
    },
  );
  ...
  server.registerTool(
    'list_geti_ui_charts',
    {
      ...
      description:
        'Lists all chart components with name, type (composition or primitive), and description.',
      inputSchema: {
        type: z
          .enum(['composition', 'primitive'])
          .optional()
          .describe('Filter by chart type'),
      },
    },
    async ({ type }) => {
      ...
      return {
        content: [
          { type: 'text' as const, text: JSON.stringify(charts, null, 2) },
        ],
      };
    },
  );
