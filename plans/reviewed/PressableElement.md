...
| #   | Issue                                                                                                                                                                                                                                                                                                                                                                                                        | Location                     |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------- |
| C1  | **`any` cast on `pressableProps`.** Lines 31–36 build a `pressableProps: any` object with explicit `(props as any)[key]` casts. This is a complete type-safety escape hatch in a component that could instead use a proper `Pick` or `Omit` utility type. ⏳ _Still open — `any` cast remains._                                                                                                              | `PressableElement.tsx:31-36` |
| C2  | **Duplicate interactive role.** The inner `<div>` has `role="button"` hardcoded while being wrapped in `<Pressable>`. React Aria's `Pressable` already renders an element with press semantics. Adding a second `role="button"` inside it creates a nested interactive element, which violates ARIA rules (interactive controls must not be nested). ✅ **Fixed:** `role="button"` removed from inner `div`. | `PressableElement.tsx:47`    |
| C3  | **`tabIndex={0}` on inner `div`** creates a second focusable element inside `Pressable`. The outer `Pressable` element is already focusable. This results in two tab stops for a single logical control. ✅ **Fixed:** `tabIndex={0}` removed from inner `div`.                                                                                                                                              | `PressableElement.tsx:48`    |
...
| #   | Issue                                                                                                                                                                                                                                                                                                                                                               | Location                     |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| A1  | **Nested interactive roles.** `<Pressable>` renders a focusable element with implicit button semantics; `<div role="button" tabIndex={0}>` inside it is a second interactive element. Screen readers will announce both. Remove `role="button"` and `tabIndex={0}` from the inner `div`. ✅ **Fixed:** `role="button"` and `tabIndex={0}` removed from inner `div`. | `PressableElement.tsx:41-49` |
| A2  | **No `aria-label` prop or documentation.** Since the component wraps arbitrary children, a consumer passing only icon children (no text) will produce an unlabelled button. The interface should include an optional `aria-label` forwarded to `Pressable`. ⏳ _Still open — no `aria-label` prop added._                                                           | `PressableElement.tsx:6-11`  |
...
| #   | Issue                                                                                                                                                                                                                                                | Location                      |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| T1  | **No test file.** `PressableElement` has no `PressableElement.test.tsx`. A component with custom prop-filtering logic, CSS truncation behaviour, and press/double-click semantics absolutely needs unit tests. ⏳ _Still open — no test file added._ | `PressableElement/` directory |

---

## Specific Fixes Required

1. **Remove `role="button"` and `tabIndex={0}` from the inner `div`.** Let `Pressable` own the interactive semantics. ✅ **Fixed.**
2. **Add `aria-label` to `PressableElementProps`** and forward it to `<Pressable>`. ⏳ _Still open._
3. **Replace `any` prop-filtering loop** with a typed Omit/Pick approach or use a utility like `splitProps` from Spectrum utils. ⏳ _Still open._
4. **Create `PressableElement.test.tsx`** with: render test, `onPress` callback test, `onDoubleClick` callback test, `isTruncated` CSS application test, and keyboard (`Enter`/`Space`) press test. ⏳ _Still open — no test file._
5. **Document `onDoubleClick` limitation** for touch environments. ⏳ _Still open._
