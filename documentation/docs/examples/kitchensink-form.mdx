...
<ExampleCard title="Text Inputs" code={`<TextField label="Username" placeholder="Enter username" />`}>
...
<ExampleCard title="Selection Controls" code={`<Switch>Dark mode</Switch>`}>
  <Grid columns={['1fr', '1fr']} gap="size-300" UNSAFE_style={{ alignItems: 'start' }}>
    <Flex direction="column" gap="size-250">
      <div>
...
<ExampleCard title="Sliders" code={`<Slider label="Opacity" defaultValue={60} minValue={0} maxValue={100} isFilled />`}>
...
<ExampleCard title="File Controls" code={`<FileTrigger><Button variant="accent">Upload file</Button></FileTrigger>`}>
  <Flex direction="column" gap="size-250">
    <div>
...
<ExampleCard title="Form" code={`<Form aria-label="Sample form">...</Form>`}>
