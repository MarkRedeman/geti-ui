...
| 2.3 | 🟡 Low   | The `<p className={styles.hint}>` for `isNewPassword` has no ARIA association. It should be `role="note"` or associated to the input via `aria-describedby`.                                                                                                                                                                                              |
...
| #   | Severity   | Finding                                                                                                                                                                    |
| --- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 4.1 | 🔴 Missing | No test verifying `validationState="invalid"` is set when `error` prop is provided — because it currently _isn't_ (see 1.2). This is a failing test waiting to be written. |
| 4.2 | 🔴 Missing | No test that `aria-describedby` on the input points to the error message.                                                                                                  |
...
1. **Critical: When `error` is set, also pass `validationState="invalid"` and `errorMessage={error}` to Spectrum**, replacing the manual `<span role="alert">` with Spectrum's built-in error rendering. This fixes `aria-describedby`, invalid border colouring, and the dual-surface confusion at once. _(High — correctness + a11y)_
2. **Remove the wrapping `<div>` or replace with `<Flex direction="row">`** so Spectrum's width/layout props are not broken. _(Medium — layout contract)_
3. **Associate `isNewPassword` hint** to input via `aria-describedby` or give it `role="note"`. _(Medium — a11y)_
4. **Replace hard-coded `px` values** with Spectrum spacing tokens. _(Low — styling)_
5. **Write test** for `aria-invalid` / `aria-describedby` wiring when `error` is set. _(High — a11y coverage)_
