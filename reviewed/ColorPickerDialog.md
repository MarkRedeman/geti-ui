...
| 2.3 | 🟠 Medium | The `ColorSwatch` inside the trigger button has no `aria-label` — it will be announced by some screen readers as an unlabelled image. Since it's decorative (the button's `aria-label` already describes the purpose), it should have `aria-hidden="true"`. |
...
| 2.5 | 🟡 Low    | No `aria-label` on any of the preset `ColorSwatch` items in `ColorSwatchPicker`. Each should have an accessible name (e.g. "Red", "Green").                                                                                                                 |
...
- [ ] **Fix Critical**: Add `useEffect` to sync `colorProp` → `color` state (`Critical`).
- [ ] **Fix High**: Add copyright header (`High`).
- [ ] **Fix High**: Import from library wrappers not directly from `@adobe/react-spectrum` (`High`).
- [ ] **Fix Medium**: Add `aria-hidden="true"` to trigger's decorative `ColorSwatch` (`Medium`).
- [ ] **Fix Medium**: Add `aria-label` to all preset `ColorSwatch` items (`Medium`).
- [ ] **Fix Medium**: Tighten `...rest` type to only `isOpen` + `onOpenChange` (`Medium`).
- [ ] Add open-dialog, Confirm, and Cancel tests (`Critical`).
- [ ] Add `onColorChange` + `isOpen` Storybook stories (`Medium`).
