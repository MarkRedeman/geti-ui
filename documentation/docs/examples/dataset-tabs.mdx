...
This example shows a dataset-focused tabs pattern built with `@geti-ai/ui` only.

- The active tab exposes a small actions menu (`Edit`, `Delete`, `Export`, `Import`).
- If there are too many tabs, extra items are collapsed into a `Picker`.
- An `Add dataset` button sits at the end of the tab strip.
- The data is mocked, and the pattern is reusable for any entity list (not just datasets).

## Custom dataset tabs pattern
...
type TabsWithOverflowProps<T> = {
  ...
  renderTab: (item: T, isActive: boolean) => JSX.Element;
  renderPanel: (item: T) => JSX.Element;
  ...
  useEffect(() => {
    ...
    const recomputeVisibleTabs = () => {
      ...
      const nextVisible = Math.max(
        minVisibleTabs,
        Math.floor(availableForTabs / estimatedTabWidth)
      );
...
- `TabsWithOverflow<T>` handles width-aware overflow and keeps the selected tab visible.
- `MenuTab` is a focused building block for active-tab action menus.
- You compose `renderTab` and `renderPanel` to reuse the same shell for datasets, models, workspaces, or other entities.
