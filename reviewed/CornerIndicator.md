...
| W2  | **Position of the dot (`top="size-50" right="size-50"`) will overlap the child.** For the default `size-50` (4px) dot this is intentional, but larger children with content near their top-right corner may have the indicator completely hidden behind the content z-index. No `zIndex` is set. | `CornerIndicator.tsx:19-22` |
...
| #   | Issue                                                                                                                                                                                  | Location                   |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| T1  | **Single render test only.** The test asserts children render but does not test: the indicator's presence/absence based on `isActive`, or the visual (CSS `display`) state difference. | `CornerIndicator.test.tsx` |
| T2  | **No test for `isActive={false}`** — the component's only conditional behaviour (hiding the indicator) is not verified.                                                                | `CornerIndicator.test.tsx` |

---

## Specific Fixes Required

1. **Add accessible `label` prop** — when `isActive` is true, render a `<span>` with `className="spectrum-VisuallyHidden"` (or equivalent) announcing the status (e.g., `"Notification pending"`).
2. **Add `color` prop** (`BackgroundColorValue`) to allow customising the dot colour.
3. **Add `isActive={false}` test** asserting the indicator element is not in the document.
4. **Add `argTypes`** to stories for `isActive` and the new `color` prop.
5. **Replace placeholder child** in stories with a realistic element (ActionButton, PhotoPlaceholder, etc.).
